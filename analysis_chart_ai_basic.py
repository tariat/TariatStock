...
class LLMTradingStrategy(Strategy):
    ...
    def init(self, *args, trade_date=None, **kwargs):
        """전략 초기화"""
    ...
    def call_llm_with_retry(self, market_data_text, current_price, position_status):
    ...
    def get_market_data_text(self, lookback=48):
        ...
        if len(self.data) < lookback:
            return ""
        return str(self.data.df.iloc[-lookback:].to_dict('list'))
    ...
    def get_llm_decision(self):
        ...
        try:
            ...
            # LLM 호출
            response = self.call_llm_with_retry(market_data_text, current_price, position_status)
    ...
    def next(self):
        ...
        now_dt = self.data.index[-1]        
        current_price = self.data.Close[-1]

        if self.trade_date and now_dt.date() < self.trade_date:
            return 
        ...
        # 거래 실행
        if decision.upper().find("BUY")>-1 and not self.position:
            # 매수: 손절매를 5% 아래로 설정
            self.buy(sl=0.95 * current_price)
            print(f"매수 실행: {current_price:.0f} (손절매: {0.95 * current_price:.0f})")
            
        elif decision.upper().find("SELL")>-1 and self.position:
            # 매도
            self.position.close()
...
def run_backtest(stock_code: str, date: str, initial_cash: int = 10000000):
    ...
    try:
        ...
        # 백테스팅 실행
        print("\n백테스팅 실행 중...")
        results = bt.run(trade_date=datetime.strptime("20250516", "%Y%m%d").date())
...
if __name__ == "__main__":
    ...
    print("\n백테스팅 완료!")
    with open("result_summary.pkl", "wb") as f:
